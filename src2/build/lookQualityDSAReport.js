...
var qualityTrTemplate = new XTemplate($('#J_quality_tr_template').html());
...
$(document).ready(function () {
  (function ($) {
    ...
    let openType = $('input[name="openType"]').val(); //look-查看 review-评估
    ...
    function bindClickAction() {
      ...
      //提交质量评估评分
      $('#J_submit_total_grade').on('click', function () {
        //校检是否为空
        if (checkGradeIsNull()) {
          alert('所有的评分项均为必填项，请填写完整之后再提交！');
          return false;
        }

        let observeId = $('input[name="ObserveReportId"]').val();
        let schemeId = $('input[name="schemeId"]').val();
        let qualityHandGradeTotal = $('.table-count-total-hand-grade').val(); //打分总分
        let qualityHandGradeJSON = []; //[{itemName:'医院名称',itemInfo:'评分项说明',itemRuleGrade:'评分项设定分值'，itemHandGrade:'评分项打分分数'},{},{},...]
        $('.quality-items-tr').each(function (index, element) {
          let item = {};
          item.itemName = $(this).find('.grade-td-itemName').data('itemname');
          item.itemInfo = $(this).find('.grade-td-itemInfo').data('iteminfo');
          item.itemRuleGrade = $(this).find('.grade-td-itemRuleGrade').data('itemrulegrade');
          item.itemHandGrade = $(this).find('.table-hand-grade').val();
          qualityHandGradeJSON.push(item);
        });

        $.ajax({
          url: window.localStorage.backSystemUrl + '/submit/DSA/report/quality/evaluate/data/by/observeId',
          headers: {
          ...
          data: {
            observeId: observeId,
            schemeId: schemeId,
            qualityHandGradeTotal: qualityHandGradeTotal,
            qualityHandGradeJSON: JSON.stringify(qualityHandGradeJSON)
          },
          success: function (result) {
            let resultJson = JSON.parse(result);
            if (resultJson.successful && resultJson.object) {
              window.open('/quality/submit/quality/result/page?observeId=' + observeId
                + '&accidentId=' + $('input[name="AccidentReportId"]').val()
                + '&isAccident=' + $('input[name="isAccident"]').val(), '_self');
            } else {
    ...
    function checkGradeIsNull() {
      let result = false;
      $('.table-hand-grade').each(function (index, element) {
        if (isNull($(this).val())) {
          result = true;
        }
      });
    ...
    if (openType === 'review') {
      //如果openType===review，从质量评估方案实体中获得属数据
      $.ajax({
        ...
        method: 'get',
        data: {
          id: $('input[name="schemeId"]').val()
        },
        success: function (result) {
          let resultJson = JSON.parse(result);
          if (resultJson.successful && resultJson.object) {
            let html = qualityTrTemplate.render({
              qualityList: JSON.parse(resultJson.object.ruleGradeJSON),
              totalRuleGrage: countTotalRuleGrade(JSON.parse(resultJson.object.ruleGradeJSON))
            });
      ...
      //如果openType===look，直接从监测表实体中获得属数据
      $.ajax({
        ...
        method: 'get',
        data: {
          observeId: $('input[name="ObserveReportId"]').val()
        },
        success: function (result) {
          let resultJson = JSON.parse(result);
          if (resultJson.successful && resultJson.object) {
            let html = qualityTrTemplate.render({
              qualityList: JSON.parse(resultJson.object.qualityHandGradeJSON),
              qualityHandGradeTotal: resultJson.object.qualityHandGradeTotal,
              totalRuleGrage: countTotalRuleGrade(JSON.parse(resultJson.object.qualityHandGradeJSON))
            });
            $('.quality-tbody').html(html);
          } else {
            toastr.error('服务器异常，请刷新页面重试！');
    ...
    //计算规则总分：[{itemName:'医院名称',itemInfo:'评分项说明',itemRuleGrade:'评分项设定分值'，itemHandGrade:'评分项打分分数'},{},{},...]
    function countTotalRuleGrade(gradeJsonList) {
      let total = 0;
      for (let x in gradeJsonList) {
        total = total + parseInt(gradeJsonList[x].itemRuleGrade);
      }
      return total;
    }
